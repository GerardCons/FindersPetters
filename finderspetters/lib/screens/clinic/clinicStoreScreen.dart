...
class ClinicStoreScreen extends StatefulWidget {
  final String storeId;
  const ClinicStoreScreen({Key? key, required this.storeId});

  @override
  State<ClinicStoreScreen> createState() => _ClinicStoreScreenState();
}

class _ClinicStoreScreenState extends State<ClinicStoreScreen> {
  final List<Clinic> _places = [];
  ...
  late Map<String, dynamic> storeSched;
  ...
  Future<void> _calculateDistances() async {
    ...
    for (final place in _places) {
      ...
      setState(() {
        ...
        storeId = place.id;
        storeSched = place.availableTime;
  ...
  bool _checkIsOpenNow(Map<String, dynamic> schedule) {
    ...
    final weekday = DateFormat('EEE').format(currentday);
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
        .collection('Clinic Directory')
        .doc(widget.storeId.trim())
        .get()
        .then((doc) {
      if (doc.exists) {
        final data = doc.data();
        final clinicPlace = Clinic.fromJson(data!);
        _places.add(clinicPlace);
        _calculateDistances();
      } else {
        ...
        leading: IconButton(
            onPressed: () {
              Navigator.of(context).pushReplacement(MaterialPageRoute(
                  builder: (context) => ClinicScreenWidget()));
            },
            icon: Icon(
              Icons.arrow_back,
              color: Colors.black,
